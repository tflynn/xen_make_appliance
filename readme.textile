...
p. The dropbox account is used to hold generated configuration files so that they are accessible during initialization of a VM. Any other mechanism that provides a read/write store could be equally well used. The read side of the store should be visible at an HTTP URL.

p. Install "Dropbox for Linux":http://www.dropbox.com/downloading?os=lnx. Probably connected it to a non-VM instance (this is the only combination tested).
...
* xen_create_appliance.config - A file containt site-specific configuration information for this script. (A sample is provided below.)
